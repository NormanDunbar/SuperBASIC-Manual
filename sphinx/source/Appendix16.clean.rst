...
is addressed, causing some incompatibility problems.
...
In order to retain compatibility with older software, the Aurora
...
..  tabularcolumns::    |l|p{.8\linewidth}|
...
SMSQ/E can be forced to overcome any incompatibility problems by
...
QL for compatibility. Attempting to read the byte stored at $18043 will
...
This is similar to MODE 4 under the Standard QL Colour Drivers and is
provided for compatibility. However all 8 standard MODE 8 colours are
actually available. See INK for a list of the standard MODE 8 colours.
...
colours. The following table details the PAL Colour Value and Native
Colour Value for each colour which need to be used for INK and similar
commands (**NOTE** this does not correspond with the original QL colour
...
achieve that colour (*NOTE* this does not correspond with the original QL
...
about some compatibility issues.
...
PROGS, which uses scalable fonts for output.
